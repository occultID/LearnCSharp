 ...
 * 适配器模式的优点：
 *  1. 可以在不修改现有代码的情况下，使用不兼容的接口。
 *  2. 可以将多个不同接口的类组合在一起，形成一个统一的接口。
 *  3. 可以将现有的类与新的接口进行适配，避免修改现有代码。
 *  4. 可以将适配器类与现有类分离，使得适配器类可以独立于现有类进行扩展。
 * 适配器模式的缺点：
 *  1. 增加了系统的复杂性，因为需要创建适配器类。
 *  2. 适配器类可能会导致性能下降，因为需要进行额外的转换。
 *  3. 适配器类可能会导致代码的可读性下降，因为需要理解适配器类的实现。
 *  4. 适配器类可能会导致代码的可维护性下降，因为需要维护适配器类的实现。
 * 适配器模式的主要组成部分包括：
 *  1. 目标接口（Target）：定义客户端所期望的接口。
 *  2. 被适配的类（Adaptee）：定义一个接口，但不符合客户端的期望。
 *  3. 适配器类（Adapter）：实现目标接口，并持有被适配的类的实例。
...
namespace LearnCSharp.DesignPattern.LearnAdapterSpace
{
    ...
    {
        public string OriginalProperty { get; set; } = "原始类的属性";
        public void OriginalMethod()
        {
            Console.WriteLine("原始类的方法");
        }
    }
    ...
    public class OriginalAdapter:Original, ITarget
    {
        public string TargetProperty { get; set; } = "适配器类的属性";
